...
extension RecoveryExerciseLabel on RecoveryExercise {
  ...
  bool get timed =>
      this == RecoveryExercise.abdominalActivation ||
...
class RecoveryDose {
  ...
  factory RecoveryDose.fromJson(Map<String, dynamic> json) => RecoveryDose(
    ...
    rangePercent: ((json['rangePercent'] as num?)?.toInt() ?? 50).clamp(
      10,
      100,
    ),
  );
...
class RecoveryObservation {
  ...
  const RecoveryObservation({
    ...
    this.symptoms = const {},
    this.delayedWorse = false,
    this.nextMorningWorse = false,
    this.provoking = const {},
  });
  bool get worse =>
      delayedWorse || nextMorningWorse || function == RecoveryFunction.worse;
  bool get urgent => symptoms.any(
    const {
      PainTag.weakness,
  ...
  Map<String, dynamic> toJson() => {
    ...
    'delayedWorse': delayedWorse,
    'nextMorningWorse': nextMorningWorse,
    'provoking': provoking.map((e) => e.name).toList(),
  };
  factory RecoveryObservation.fromJson(Map<String, dynamic> j) =>
      RecoveryObservation(
        date: DateTime.parse(j['date'] as String),
        pain: ((j['pain'] as num?)?.toInt() ?? 0).clamp(0, 10),
        sittingMinutes: ((j['sittingMinutes'] as num?)?.toInt() ?? 0).clamp(
          0,
          1440,
        ),
        function: _enum(
          RecoveryFunction.values,
          j['function'],
          RecoveryFunction.unchanged,
        ),
        symptoms: PainTag.values
            .where((v) => (j['symptoms'] as List? ?? []).contains(v.name))
            .toSet(),
        delayedWorse: j['delayedWorse'] == true,
        nextMorningWorse: j['nextMorningWorse'] == true,
        provoking: RecoveryExercise.values
            .where((v) => (j['provoking'] as List? ?? []).contains(v.name))
            .toSet(),
      );
}
...
class RecoveryProgram {
  ...
  const RecoveryProgram({
    this.phase = RecoveryPhase.flareUp,
    this.bikePaused = true,
    this.reviewRequired = false,
    this.assessedAt,
    this.flareAt,
    this.observations = const [],
    this.selected = const {RecoveryExercise.abdominalActivation},
    this.pausedExercises = const {},
    this.doses = const {},
    this.pendingSession,
    this.pendingCompleteDose = false,
    this.pendingWorse = false,
    this.pendingExercises = const {},
    this.toleratedExposures = 0,
    this.sessions = const [],
  });
  RecoveryObservation? get latest =>
  ...
  RecoveryDose dose(RecoveryExercise exercise) =>
      doses[exercise] ?? const RecoveryDose();
  bool checkedToday(DateTime date) =>
      latest != null &&
      sameRecoveryDay(latest!.date, date) &&
      (flareAt == null || !latest!.date.isBefore(flareAt!));
  bool get trainingBlocked =>
      reviewRequired || latest?.symptoms.isNotEmpty == true;
  ...
  RecoveryProgram copyWith({
    ...
    DateTime? flareAt,
    List<RecoveryObservation>? observations,
    Set<RecoveryExercise>? selected,
    Set<RecoveryExercise>? pausedExercises,
    Map<RecoveryExercise, RecoveryDose>? doses,
    DateTime? pendingSession,
    bool clearPending = false,
    bool? pendingCompleteDose,
    bool? pendingWorse,
    Set<RecoveryExercise>? pendingExercises,
    int? toleratedExposures,
    List<DateTime>? sessions,
  }) => RecoveryProgram(
    phase: phase ?? this.phase,
    bikePaused: bikePaused ?? this.bikePaused,
    reviewRequired: reviewRequired ?? this.reviewRequired,
    assessedAt: clearAssessment ? null : assessedAt ?? this.assessedAt,
    flareAt: flareAt ?? this.flareAt,
    observations: observations ?? this.observations,
    selected: selected ?? this.selected,
    pausedExercises: pausedExercises ?? this.pausedExercises,
    doses: doses ?? this.doses,
    pendingSession: clearPending ? null : pendingSession ?? this.pendingSession,
    pendingCompleteDose: clearPending
        ? false
        : pendingCompleteDose ?? this.pendingCompleteDose,
    pendingWorse: clearPending ? false : pendingWorse ?? this.pendingWorse,
    pendingExercises: clearPending
        ? const {}
  ...
  Map<String, dynamic> toJson() => {
    ...
    'doses': {
      for (final entry in doses.entries) entry.key.name: entry.value.toJson(),
    },
    'pendingSession': pendingSession?.toIso8601String(),
    'pendingCompleteDose': pendingCompleteDose,
    'pendingWorse': pendingWorse,
  ...
  factory RecoveryProgram.fromJson(Map<String, dynamic> j) {
    Set<RecoveryExercise> choices(String key) => RecoveryExercise.values
        .where((v) => (j[key] as List? ?? []).contains(v.name))
        .toSet();
    return RecoveryProgram(
      ...
      observations: (j['observations'] as List? ?? [])
          .whereType<Map>()
          .map((o) => RecoveryObservation.fromJson(o.cast<String, dynamic>()))
          .toList(),
      selected: j.containsKey('selected')
          ? choices('selected')
          : const {RecoveryExercise.abdominalActivation},
      pausedExercises: choices('pausedExercises'),
      doses: {
        for (final e in RecoveryExercise.values)
          if ((j['doses'] as Map?)?[e.name] is Map)
            e: RecoveryDose.fromJson(
              ((j['doses'] as Map)[e.name] as Map).cast<String, dynamic>(),
            ),
      },
      pendingSession: DateTime.tryParse(j['pendingSession']?.toString() ?? ''),
      pendingCompleteDose: j['pendingCompleteDose'] == true,
      pendingWorse: j['pendingWorse'] == true,
      pendingExercises: choices('pendingExercises'),
