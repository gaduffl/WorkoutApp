...
void main() {
  ...
  RecoveryObservation observation({
    ...
    date: day, pain: 1, sittingMinutes: 30,
    function: improved ? RecoveryFunction.better : RecoveryFunction.unchanged,
    symptoms: symptoms, delayedWorse: worse,
  );
  ...
  test('unchanged and incomplete work cannot earn an increase', () {
    var state = engine.observe(const RecoveryProgram(), observation(improved: false));
    state = engine.complete(state, date: day, completeDose: false, worse: false);
    state = engine.observe(state, RecoveryObservation(
      date: day.add(const Duration(days: 1)), pain: 1, sittingMinutes: 30,
  ...
  test('worsening pauses the offending exercise even at minimum dose', () {
    final state = engine.flare(const RecoveryProgram(), day,
  ...
  test('symptoms, choices and independent doses survive persistence', () {
    final state = engine.observe(const RecoveryProgram(
      selected: {RecoveryExercise.abdominalActivation, RecoveryExercise.floorPress},
  ...
  test('full exposures require next-day feedback and phases only move explicitly', () {
    var state = engine.observe(const RecoveryProgram(), observation());
    for (var n = 0; n < 2; n++) {
      final session = day.add(Duration(days: n));
      state = engine.complete(state, date: session, completeDose: true, worse: false,
          exercises: {RecoveryExercise.abdominalActivation});
      expect(engine.canAdvance(state, session), isFalse);
      state = engine.observe(state, RecoveryObservation(
        date: session.add(const Duration(days: 1)), pain: 1, sittingMinutes: 30,
  ...
  test('dose changes use equipment steps and never increase a paused movement', () {
    var state = engine.observe(const RecoveryProgram(
      phase: RecoveryPhase.hingeReturn, toleratedExposures: 2,
      selected: {RecoveryExercise.gluteBridge},
    ), observation());
    expect(() => engine.adjustDose(state, RecoveryExercise.gluteBridge,
        const RecoveryDose(load: 6), day), throwsStateError);
    state = engine.recordAssessment(state, day);
    expect(() => engine.adjustDose(state, RecoveryExercise.gluteBridge,
        const RecoveryDose(load: 5), day), throwsArgumentError);
    expect(() => engine.adjustDose(state, RecoveryExercise.gluteBridge,
        const RecoveryDose(load: 10), day), throwsArgumentError);
    final changed = engine.adjustDose(state, RecoveryExercise.gluteBridge,
        const RecoveryDose(load: 6), day);
    ...
    expect(() => engine.adjustDose(state, RecoveryExercise.gluteBridge,
        const RecoveryDose(reps: 6), day), throwsStateError);
  });

  test('minimum-dose worsening stays paused until an explicit settled re-trial', () {
    var state = engine.complete(engine.observe(const RecoveryProgram(), observation()),
      date: day, completeDose: true, worse: true,
      exercises: {RecoveryExercise.abdominalActivation});
    expect(state.sessions, hasLength(1));
    expect(engine.due(state, day), isFalse);
    expect(engine.allowed(state, RecoveryExercise.abdominalActivation, alternative: false), isFalse);
    ...
    state = engine.retrial(state, RecoveryExercise.abdominalActivation, day.add(const Duration(days: 1)));
