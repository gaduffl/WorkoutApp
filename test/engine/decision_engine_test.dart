...
void main() {
  ...
  List<SessionLog> floorSatisfiedLogs() => [
    buildLog(SessionTypeId.s1, today.subtract(const Duration(days: 2)), {
      FloorCategory.strength,
    }),
    buildLog(SessionTypeId.s4, today.subtract(const Duration(days: 4)), {
      FloorCategory.strength,
    }),
    buildLog(SessionTypeId.s3, today.subtract(const Duration(days: 3)), {
      FloorCategory.intensity,
    }),
    buildLog(SessionTypeId.s6, today.subtract(const Duration(days: 2)), {
      FloorCategory.aerobic,
    }),
    buildLog(SessionTypeId.s7, today.subtract(const Duration(days: 5)), {
      FloorCategory.intensity,
    }),
    buildLog(SessionTypeId.s7, today.subtract(const Duration(days: 7)), {
      FloorCategory.intensity,
    }),
  ];
  ...
  test('§5 worked example: S1 with sharp lower-back pain', () {
    ...
    final hinge = trace.plan!.exercises.firstWhere(
      (e) => e.substitutedFrom == 'hinge',
    );
    ...
    final squat = trace.plan!.exercises.firstWhere(
      (e) => e.pattern == MovementPattern.squat,
    );
    expect(squat.loadTotal, lessThan(24));

    expect(
      trace.firedRuleCodes,
      containsAll([
        'ONBOARD_SUBSTITUTE',
        'PAIN_SUB_HINGE_SHARP',
        'PAIN_SUB_SQUAT_SHARP',
      ]),
    );
    expect(
      output.patchedExerciseStates['squat']!.sessionsScheduledWhileFlagged,
      1,
    );
    expect(
      output.patchedExerciseStates['hinge']!.sessionsScheduledWhileFlagged,
      1,
    );
  });

  test(
    'recovery requires a current symptom check even for forced GREEN work',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 5,
  ...
  RecoveryProgram checkedProgram({
    ...
    Set<RecoveryExercise> selected = const {
  ...
  test(
    'flare-up work does not inherit advanced ladders, jumping or cardio',
    () {
      for (final id in SessionTypeId.values) {
        ...
        expect(
          plan.exercises.where((e) => !e.isWarmup).map((e) => e.trackKey),
  ...
  test(
    'rebuilding selects only opted-in supported work across every strength family',
    () {
      for (final id in [
        SessionTypeId.s1,
        SessionTypeId.s2,
              ...
              lowerBackRecovery: LowerBackRecoveryState(
                ...
                program: checkedProgram(
                  ...
                  selected: {
                    ...
                    RecoveryExercise.supportedRow,
                    RecoveryExercise.pullUp,
                    RecoveryExercise.curl,
                  },
                  doses: {
        ...
        expect(
          work.every((e) => e.trackKey.startsWith('recovery:v2:')),
          isTrue,
        );
        expect(
          work.every(
            (e) => e.rirTarget == Rir.rir4plus && !e.progressionEligible,
          ),
          isTrue,
        );
        expect(
          work.map((e) => e.loadTotal ?? 0),
  ...
  test(
    'recovery spacing and neurological review do not substitute more work',
    () {
  ...
  test('bike pause excludes every cardio choice including forced swaps', () {
    for (final id in [SessionTypeId.s3, SessionTypeId.s6, SessionTypeId.s7]) {
      ...
      expect(
        output.trace.candidates.any(
          (c) => [
            SessionTypeId.s3,
            SessionTypeId.s6,
            SessionTypeId.s7,
          ].contains(c.sessionId),
        ),
        isFalse,
      );
    }
  ...
  test(
    'deadlift alternative emits both independent tracks and no deadlift load',
    () {
  ...
  test(
    'rest-day pain persists only affected normal tracks without scheduling them',
    () {
  ...
  test(
    'lower-back pain persists both lower tracks on a cardio safety swap',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 35,
  ...
  test(
    'persisted check-in pain blocks S3 and S7 next day without a re-tap',
    () {
      final restDay = decisionEngine.decide(
        buildInput(
          time: 0,
          subjective: 4,
          pain: [
  ...
  test(
    'persisted sharp hip swaps next-day leg-heavy work without a re-tap',
    () {
      final restDay = decisionEngine.decide(
        buildInput(
          time: 0,
  ...
  group('RED-day swaps', () {
    test(
      'a RED strength session runs as a technique session, not a type swap',
      () {
        final input = buildInput(
          time: 35,
  ...
  test('surplus intensity is scored below target-bearing candidates', () {
    final input = buildInput(
      ...
      recoveryHistory: normalHrvHistory(),
      queueState: const QueueState(
        pointer: SessionTypeId.s1,
      ), // S5 has cycle_distance 4 -> base 10, tying S6/S7
      sessionLogs: floorSatisfiedLogs(),
    );
    final output = decisionEngine.decide(input);
  ...
  group('§11 swap session', () {
    test(
      'forcedSessionId overrides the natural winner but still runs modulation/pain steps',
      () {
        final input = buildInput(
          time: 35,
          subjective: 4,
  ...
  test('legacy weeklyFloor settings no longer change v2 scores', () {
    final normal = decisionEngine.decide(
      buildInput(
        time: 35,
        subjective: 4,
        sessionLogs: floorSatisfiedLogs(),
    ...
    final extremeLegacyFloor = decisionEngine.decide(
      buildInput(
        time: 35,
        subjective: 4,
        sessionLogs: floorSatisfiedLogs(),
    ...
    expect(
      extremeLegacyFloor.trace.plan!.sessionId,
      normal.trace.plan!.sessionId,
    );
    expect(
    ...
    expect(
      extremeLegacyFloor.trace.firedRuleCodes.where(
        (code) => code.startsWith('FLOOR_'),
      ),
      isEmpty,
    );
  });
  ...
  test(
    '§6 order: 20-min YELLOW stacks compression then recovery volume reduction',
    () {
      final input = buildInput(
        time: 20,
        subjective: 3,
  ...
  test(
    '§5 Step 6: RED technique session is a swap — no queue credit, pointer unchanged',
    () {
      final input = buildInput(
        time: 35,
        subjective: 1, // forces RED
  ...
  test(
    '§6.5: an active deload actually changes the prescription (60% load, half sets, RIR>=4)',
    () {
      ...
      final squat = output.trace.plan!.exercises.firstWhere(
        (e) => e.pattern == MovementPattern.squat && !e.isWarmup,
      );
      ...
      final hinge = output.trace.plan!.exercises.firstWhere(
        (e) => e.pattern == MovementPattern.hinge && !e.isWarmup,
      );
  ...
  test(
    '§7.2: a sharp freeze keeps substituting on later days without re-tapping the body map',
    () {
      final states = baseStates();
      ...
      expect(
        ...
        isNot(contains('ONBOARD_SUBSTITUTE')),
      );
      final hinge = output.trace.plan!.exercises.firstWhere(
        (e) => e.substitutedFrom == 'hinge',
      );
  ...
  test(
    '§2.5 warm-up protocol: 40/60/80 ramp before the first compound, feeders after',
    () {
      final input = buildInput(
        time: 35,
        subjective: 4,
  ...
  test('the ATG block replaces general prep, not the first-compound ramp', () {
    ...
    final compressed = decisionEngine.decide(
      buildInput(
        time: 20,
        subjective: 4,
        recoveryHistory: normalHrvHistory(),
  ...
  test(
    'current knee pain replaces S4 ATG loading with equal-time pain-aware prep',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 60,
  ...
  test(
    '§5 Step 7 "60->35": a 60-min session in a 35-min slot drops the accessory block and cuts compound volume',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 35,
      ...
      final ext = decisionEngine.decide(
        buildInput(
          time: 60,
          subjective: 4,
          recoveryHistory: normalHrvHistory(),
      ...
      expect(
        ext.trace.plan!.exercises.any(
          (e) => !e.isWarmup && e.pattern == MovementPattern.coreGrip,
        ),
        isTrue,
      );
      expect(ext.trace.plan!.timeCompressed, isFalse);
      expect(
        ext.trace.plan!.plannedWorkSets,
        greaterThan(output.trace.plan!.plannedWorkSets),
      );
    },
  );

  test(
    '§2.6: work exercises carry their PowerBlock achievable-total steps',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 35,
      ...
      expect(
        squat.loadSteps,
        containsAllInOrder([6, 9, 10, 12, 15, 18, 20, 21, 24, 25]),
      );
  ...
  test(
    '§2.5: consecutive compound work exercises are paired into superset groups',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 60,
      ...
      final work = output.trace.plan!.exercises
          .where((e) => !e.isWarmup)
          .toList();
      final compounds = work
          .where((e) => e.pattern.patternClass == PatternClass.compound)
          .toList();
      ...
      final accessory = work.firstWhere(
        (e) => e.pattern == MovementPattern.coreGrip,
      );
  ...
  test(
    '§12 travel mode: ladders resolve to bodyweight, arm accessories drop out',
    () {
      final s1 = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 4,
          recoveryHistory: normalHrvHistory(),
      ...
      final s2 = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 4,
          recoveryHistory: normalHrvHistory(),
      ...
      expect(
        s2Names,
        containsAll(const ['Prone lat pull-down', 'Prone W-row']),
      );
      expect(
        s2Names.any((name) => name.contains('bar') || name.contains('Table')),
        isFalse,
      );

      final s4 = decisionEngine.decide(
        buildInput(
          time: 60,
          subjective: 4,
          recoveryHistory: normalHrvHistory(),
      ...
      expect(
        s4.trace.plan!.exercises
            ...
            .any(
              (name) =>
                  name.contains('treadmill') || name.contains('slant-board'),
            ),
        isFalse,
      );

      final s5 = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 4,
          recoveryHistory: normalHrvHistory(),
      ...
      expect(
        s5.trace.plan!.exercises.any((e) => e.name == 'Alternating DB curl'),
        isFalse,
      );
      final s5Work = s5.trace.plan!.exercises
          .where((e) => !e.isWarmup)
          .toList();
      expect(
        s5Work.map((e) => e.name),
  ...
  test('travel mode keeps pain adjustments conservative without equipment', () {
    ...
    expect(work.every((e) => e.loadTotal == null), isTrue);
    expect(
      work.where((e) => e.instruction?.contains('pain-free range') == true),
      isNotEmpty,
    );
    expect(
      work
          .where((e) => e.instruction?.contains('pain-free range') == true)
          .every((e) => e.rirTarget == Rir.rir4plus),
      isTrue,
    );
  });

  test(
    '§6.6: a detraining-adjusted load is marked to persist on completion',
    () {
      final states = baseStates();
      states['hinge'] = ExerciseState(
        trackKey: 'hinge',
        pattern: MovementPattern.hinge,
        currentLoad: 100,
        lastTrainedDate: today.subtract(
  ...
  test(
    'YELLOW and RED detraining plans retain their emitted safe baseline marker',
    () {
      for (final subjective in [3, 1]) {
        ...
        final hinge = output.trace.plan!.exercises.firstWhere(
          (exercise) =>
              exercise.pattern == MovementPattern.hinge && !exercise.isWarmup,
        );
  ...
  test('first-ever work uses the minimum load without comeback narration', () {
    ...
    expect(
      output.trace.firedRuleCodes,
      isNot(contains('DETRAIN_ADJUST_SQUAT')),
    );
    expect(
      squat.loadTotal,
      6,
      reason: 'the minimum single-DB load remains the onboarding load',
    );
  ...
  test(
    '§6.3: global deload also applies to a newly created scheduled state',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 35,
          ...
          exerciseStates: {'squat': baseStates()['squat']!},
          queueState: const QueueState(pointer: SessionTypeId.s1),
          sessionLogs: floorSatisfiedLogs(),
        ),
      );
  ...
  test(
    '§6.3: the same at-or-above-three RED cluster never retriggers completed tracks',
    () {
      ...
      expect(output.patchedExerciseStates['hinge']!.deloadSessionsRemaining, 0);
      expect(output.patchedExerciseStates['squat']!.deloadSessionsRemaining, 1);
    },
  );
  ...
  test(
    '§6.3: automatic global deload persists for unscheduled tracks after the trigger ages out',
    () {
      final triggered = decisionEngine.decide(
        buildInput(
          time: 0,
          subjective: 1,
          checkinHistory: [
      ...
      expect(
        triggered.patchedExerciseStates[dip.trackKey]!.deloadSessionsRemaining,
        2,
      );

      final afterTriggerAgedOut = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 4,
  ...
  test(
    'sharp hip protection overrides a manually forced leg-heavy session',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 35,
  ...
  test('pain re-entry is exactly 1 x 8 at the resolved 50% test load', () {
    ...
    final squat = output.trace.plan!.exercises.firstWhere(
      (exercise) =>
          exercise.pattern == MovementPattern.squat && !exercise.isWarmup,
    );
    expect(squat.sets, 1);
  ...
  test(
    'travel movement check does not claim to complete the loaded pain re-entry test',
    () {
      final states = baseStates();
      states['squat'] = ExerciseState(
        trackKey: 'squat',
      ...
      expect(
        squat.instruction,
        contains('formal loaded re-entry remains pending'),
      );
      expect(
        output.trace.firedRuleCodes,
        isNot(contains('PAIN_REENTRY_TEST_SQUAT')),
      );
  ...
  test(
    'compressed travel S5 preserves its dynamic pair with viable variants',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 20,
      ...
      expect(work.map((exercise) => exercise.name), [
        'Self-resisted curl',
        'Prone Y-raise',
      ]);
  ...
  test(
    'mild wrist pain reduces every S5 named accessory by one real load step',
    () {
      final states = baseStates();
      ...
      final workByKey = {
        for (final exercise in output.trace.plan!.exercises.where(
          (value) => !value.isWarmup,
        ))
  ...
  test(
    'mild shoulder pain keeps named S5 tracks and uses single-DB reduction',
    () {
      final states = baseStates();
      for (final named in s5NamedAccessories) {
        states[named.trackKey] = ExerciseState(
  ...
  test('home core holds use seconds while wrist curls remain rep-based', () {
    final plankOutput = decisionEngine.decide(
      buildInput(
        time: 35,
        subjective: 4,
        recoveryHistory: normalHrvHistory(),
    ...
    final curlOutput = decisionEngine.decide(
      buildInput(
        time: 35,
        subjective: 4,
        recoveryHistory: normalHrvHistory(),
  ...
  test(
    'timed-hold micro-stages use position and leverage cues, never rep cues',
    () {
      const expectations = <(int, String)>[
  ...
  test(
    'recovery, deload, pain re-entry, travel, and warm-ups suppress micro cues',
    () {
      ...
      final yellow = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 3,
          queueState: const QueueState(pointer: SessionTypeId.s1),
      ...
      expect(
        yellowWork.every((exercise) => exercise.instruction == null),
        isTrue,
      );
      ...
      final deload = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 4,
          recoveryHistory: normalHrvHistory(),
      ...
      final reentry = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 4,
          recoveryHistory: normalHrvHistory(),
      ...
      final travel = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 4,
          recoveryHistory: normalHrvHistory(),
  ...
  test('every hold step has an explicit seconds prescription', () {
    ...
    for (final name in [
      ...
      expect(
        step.targetRange!.$2,
        greaterThan(step.targetRange!.$1),
        reason: name,
      );
    }
    final wristCurl = coreSteps.firstWhere(
      (step) => step.name == 'Wrist curls',
    );
    expect(wristCurl.metric, ExerciseMetric.reps);
    expect(
      wristCurl.dumbbells,
      2,
      reason:
          'Wrist curls is a bilateral rep move (one DB per wrist); using '
  ...
  test(
    'a strength template with no pain-free work returns a no-plan outcome',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 35,
  ...
  test(
    'medical escalation wording is fixed in EN/DE and bypasses AI narration',
    () async {
      const rule = FiredRule(RuleKey.painMedicalEscalation, pattern: 'squat');
      const english =
      ...
      final explanation = await const AiExplainer().dailyExplanation(
        output.trace,
        const UserSettings(anthropicApiKey: 'must-not-be-used-for-safety-rule'),
      );
  ...
  test('long base deficit yields to feasible strength deficits', () {
    ...
    final weekendLogs = [
      buildLog(SessionTypeId.s1, saturday.subtract(const Duration(days: 1)), {
        FloorCategory.strength,
      }),
      buildLog(SessionTypeId.s4, saturday.subtract(const Duration(days: 3)), {
        FloorCategory.strength,
      }),
      buildLog(SessionTypeId.s3, saturday.subtract(const Duration(days: 2)), {
        FloorCategory.intensity,
      }),
    ];
    final output = decisionEngine.decide(
  ...
  test(
    'a RED weekend S6 winner remains Zone 2 and never becomes a technique session',
    () {
      final saturday = DateTime(2026, 1, 24);
      final weekendLogs = [
        buildLog(SessionTypeId.s1, saturday.subtract(const Duration(days: 1)), {
      ...
      expect(
        output.trace.firedRuleCodes,
        isNot(contains('RED_SWAP_TECHNIQUE')),
      );
    },
  );

  test('CAROL presets keep fixed duration and full tier', () {
    final s3 = decisionEngine.decide(
      buildInput(
        time: 60,
        subjective: 4,
        todaySnapshot: RecoverySnapshot(
    ...
    final s7 = decisionEngine.decide(
      buildInput(
        time: 35,
        subjective: 4,
        todaySnapshot: RecoverySnapshot(
  ...
  test(
    'RED 20-minute safety swap stays inside the slot and is non-qualifying base work',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 20,
  ...
  test(
    'CAP_LADDER_JUMP follows awaitingUndershootCheck, not detraining regression',
    () {
      ...
      final advanced = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 4,
          todaySnapshot: RecoverySnapshot(
      ...
      final detrained = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 4,
          todaySnapshot: RecoverySnapshot(
      ...
      expect(
        detrained.trace.firedRuleCodes,
        isNot(contains('CAP_LADDER_JUMP_SQUAT')),
      );
    },
  );

  test(
    'pain resolution chooses the most restrictive flag independent of tap order',
    () {
      ...
      for (final flags in [
        ...
        expect(
          output.patchedExerciseStates['squat']!.painRegion,
          BodyRegion.kneeLeft,
        );
        expect(
          output.patchedExerciseStates['squat']!.sessionsScheduledWhileFlagged,
  ...
  test(
    'persisted escalation tags and stricter region survive a day without re-tapping',
    () {
      final first = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 4,
          pain: [
      ...
      final nextDay = today.add(const Duration(days: 1));
      final second = decisionEngine.decide(
        DecisionEngineInput(
          checkin: CheckIn(
            date: nextDay,
            timeMinutes: 35,
      ...
      final squatStillScheduled =
          second.trace.plan?.exercises.any(
            (exercise) =>
                !exercise.isWarmup && exercise.pattern == MovementPattern.squat,
          ) ??
          false;
      ...
      expect(
        second.patchedExerciseStates['squat']!.painRegion,
        BodyRegion.kneeLeft,
      );
      expect(
        second.patchedExerciseStates['squat']!.painTags,
        contains(PainTag.radiating),
      );
    },
  );
  ...
  test('exercise-state insertion order does not affect v2 target scores', () {
    ...
    for (final output in outputs) {
      ...
      expect(
        output.trace.firedRuleCodes.where(
          (code) => code.startsWith('RECENCY_'),
        ),
        isEmpty,
      );
    }
  ...
  test(
    'readiness prescriptions persist progression eligibility and exact RED RIR',
    () {
      final green = decisionEngine.decide(
        buildInput(
          time: 35,
          subjective: 4,
          todaySnapshot: RecoverySnapshot(
      ...
      final yellow = decisionEngine.decide(
        buildInput(time: 35, subjective: 3, sessionLogs: floorSatisfiedLogs()),
      );
      final red = decisionEngine.decide(
        buildInput(time: 35, subjective: 1, sessionLogs: floorSatisfiedLogs()),
      );

      final greenWork = green.trace.plan!.exercises.where(
        (exercise) => !exercise.isWarmup,
      );
      final yellowWork = yellow.trace.plan!.exercises.where(
        (exercise) => !exercise.isWarmup,
      );
      final redWork = red.trace.plan!.exercises.where(
        (exercise) => !exercise.isWarmup,
      );
      expect(
        greenWork.every((exercise) => exercise.progressionEligible),
        isTrue,
      );
      expect(
        yellowWork.every((exercise) => !exercise.progressionEligible),
        isTrue,
      );
      expect(
        redWork.every((exercise) => !exercise.progressionEligible),
        isTrue,
      );
      expect(
        redWork.every((exercise) => exercise.rirTarget == Rir.rir4plus),
        isTrue,
      );
    },
  );
  ...
  test('YELLOW and RED keep active deload work prescribed at RIR 4+', () {
    for (final subjective in [3, 1]) {
      ...
      final squat = output.trace.plan!.exercises.firstWhere(
        (exercise) => !exercise.isWarmup && exercise.trackKey == 'squat',
      );
  ...
  test(
    '20-minute S3 priority substitutes to S7 without compression trace or queue credit',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 20,
      ...
      expect(
        output.trace.firedRuleCodes,
        isNot(contains('TIME_COMPRESS_35_20')),
      );
    },
  );

  test(
    'RED 20-minute S3 applies time substitution before the S7-to-S6 safety swap',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 20,
  ...
  test('recency and queue rules describe only the actually selected plan', () {
    ...
    final forced = decisionEngine.decide(
      buildInput(
        time: 35,
        subjective: 4,
        todaySnapshot: RecoverySnapshot(
    ...
    final hipSwap = decisionEngine.decide(
      buildInput(
        time: 35,
        subjective: 4,
        pain: [
    ...
    final output = decisionEngine.decide(
      buildInput(
        ...
        sessionLogs: [
          ...
          buildLog(SessionTypeId.s1, today.subtract(const Duration(days: 1)), {
            FloorCategory.strength,
          }),
        ],
        forceQueuePointer: false,
  ...
  test(
    'floor rationale is omitted when a forced plan does not cover the pressured category',
    () {
      final output = decisionEngine.decide(
        buildInput(
          time: 35,
      ...
      expect(
        output.trace.firedRuleCodes,
        isNot(contains('FLOOR_FORCE_STRENGTH')),
      );
