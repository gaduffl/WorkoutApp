...
void main() {
  ...
  test(
    'recovery check persists resolved symptoms and blocks saved prescriptions',
    () async {
      final controller = _SettingsController(
        Repository(_SettingsMemoryDatabase()),
      );
      await controller.activateLowerBackRecovery(
        symptomOnsetDate: controller.today(),
        confirmedNoRedFlags: true,
      );
      await controller.recordRecoveryObservation(
        RecoveryObservation(
          date: DateTime.now(),
          pain: 1,
          sittingMinutes: 30,
      ...
      await expectLater(
        controller.deactivateLowerBackRecovery(),
        throwsStateError,
      );
    },
  );
  ...
  test(
    'bike and deadlift preferences invalidate saved work immediately',
    () async {
      final controller = _SettingsController(
        Repository(_SettingsMemoryDatabase()),
      );
      const bike = SessionPlan(
  ...
  test(
    'explicit recovery exit preserves the reviewed load, never the old deadlift',
    () async {
      final controller =
  ...
  test(
    'ordinary settings saves cannot overwrite recovery lifecycle state',
    () async {
      final db = _SettingsMemoryDatabase();
      final repository = Repository(db);
      final controller = _SettingsController(repository)
      ...
      expect(controller.lowerBackRecovery.preRecoveryHingeLoad, 90);
      expect(
        (await repository.loadSettings()).lowerBackRecovery.active,
        isTrue,
      );
    },
  );
  ...
  test(
    'controller refuses activation without the red-flag confirmation',
    () async {
      final controller = _SettingsController(
        Repository(_SettingsMemoryDatabase()),
      );
