...
void main() {
  ...
  test(
    'recovery logs preserve work but partial dose earns no tolerance or queue credit',
    () async {
      for (final full in [false, true]) {
        ...
        final plan = const RecoveryProgramEngine().plan(
          controller.lowerBackRecovery.program,
          date: now,
          minutes: 20,
          alternative: false,
          pain: [],
  ...
  SessionPlan strengthPlan({
    ...
    optionalRehitFinisherReserved:
        reserveRehitFinisher ??
        (id == SessionTypeId.s2 && tier == SessionTier.extended),
  );

  List<SetLog> workSets(int count, {bool painFlag = false, int value = 8}) =>
      List.generate(
        count,
        (index) => SetLog(
          trackKey: 'squat',
  ...
  test(
    'manual swap records override but travel refresh preservation does not',
    () async {
      final now = DateTime.now();
  ...
  test(
    'resetToday restores rest/cardio pain persistence and deletes new tracks',
    () async {
  ...
  test(
    'partial cardio persists without category/queue credit; full dose earns both',
    () async {
      final db = _MemoryDatabase();
      final repo = Repository(db);
      final controller = AppController(repo)
  ...
  test(
    ...
    () async {
      ...
      await expectLater(
        controller.completeSession(plan, workSets(4), durationMinutes: 35),
        throwsStateError,
      );
  ...
  test(
    'a partial primary cardio attempt cannot be logged or completed again',
    () async {
  ...
  test(
    'the explicitly eligible later-day REHIT bypasses only the primary lock',
    () async {
  ...
  test(
    ...
    () async {
      ...
      await expectLater(
        controller.logCardioSession(SessionTypeId.s7, completion: partial),
        throwsStateError,
      );
  ...
  test(
    'persisted supplemental-only work leaves every primary action unlocked',
    () async {
      final repo = Repository(_MemoryDatabase());
  ...
  test(
    'retrospective REHIT bypasses an existing primary lock without queue credit',
    () async {
      final controller = await controllerAfterStrength();
  ...
  test(
    'unplanned Zone 2 saves actual dose without changing the primary plan',
    () async {
      for (final minutes in [20, 30, 60, 95]) {
        for (final primaryState in [0, 1, 2]) {
          ...
          final logs = await controller.repo.loadSessionLogsSince(
            DateTime(2000),
          );
  ...
  test(
    'unplanned Zone 2 rejects interval completions without saving',
    () async {
      final controller = AppController(Repository(_MemoryDatabase()));
      final prescription = cardio.prescriptionFor(
        sessionId: SessionTypeId.s7,
  ...
  test('high intensity requires an authoritative current GREEN trace', () {
    ...
    expect(controller.isHighIntensityUsableNow(nowLocal: now), isFalse);
    ...
    controller.todayTrace = traceFor(now, plan);
    expect(controller.isHighIntensityUsableNow(nowLocal: now), isTrue);
  });
  ...
  test(
    ...
    () async {
      ...
      final recoveryLog = (await repo.loadSessionLogsSince(
        DateTime(2000),
      )).single;
      ...
      for (final plannedMinutes in [35, 60]) {
        ...
        final partialLog = (await partialRepo.loadSessionLogsSince(
          DateTime(2000),
        )).single;
        expect(
  ...
  test(
    'stale CAROL plans cannot be logged while travel mode is active',
    () async {
      final db = _MemoryDatabase();
      final repo = Repository(db);
      final controller = AppController(repo)
  ...
  test(
    'controller rejects a mismatched completion before persistence',
    () async {
      final db = _MemoryDatabase();
      final repo = Repository(db);
      final controller = AppController(repo);
  ...
  test(
    'controller keeps CAROL dose credit separate from preset adherence',
    () async {
      for (final id in [SessionTypeId.s3, SessionTypeId.s7]) {
        ...
        final completion = cardio.completionFromEntry(
          prescription: plan.cardioPrescription!,
          completedWorkIntervals: plan.cardioPrescription!.plannedWorkIntervals,
  ...
  test(
    'controller canonicalizes restored legacy CAROL prescriptions',
    () async {
      ...
      for (final (id, legacyPrescription, observedSeconds) in cases) {
        ...
        expect(
          log.cardioCompletion!.completedRecoverySeconds,
  ...
  test(
    'legacy cardio plan derives a prescription but still requires completion',
    () async {
      final db = _MemoryDatabase();
      final repo = Repository(db);
      final controller = AppController(repo);
      ...
      await expectLater(
        controller.completeSession(legacyPlan, const [], durationMinutes: 35),
        throwsArgumentError,
      );
      expect(await repo.loadSessionLogsSince(DateTime(2000)), isEmpty);
  ...
  test(
    'Zone 2 longer than the 60-minute plan persists its full dose',
    () async {
      final db = _MemoryDatabase();
      final repo = Repository(db);
      final controller = AppController(repo);
  ...
  test(
    'zero-value work is persisted for pain context but earns no completion or queue credit',
    () async {
      final db = _MemoryDatabase();
      final repo = Repository(db);
      final controller = AppController(repo)
  ...
  test(
    'S2 finisher persists structured REHIT and intensity only at full dose',
    () async {
  ...
  group('immediate S2 finisher safety', () {
    ...
    test('exactly 50% qualifies unless the session explicitly ended early', () {
      ...
      final exactHalf = controller.rehitFinisherEligibility(plan, workSets(2));
    ...
    test('pain, incomplete work, deload, and travel close the shared gate', () {
      ...
      expect(
        controller
            .rehitFinisherEligibility(plan, workSets(2, painFlag: true))
            .closedReasons,
    ...
    test('controller accepts finisher only for extended S2', () async {
      ...
      final unreservedController = AppController(Repository(_MemoryDatabase()));
      ...
      unreservedController.todayTrace = traceFor(
        DateTime.now(),
        unreservedExtended,
      );
      await expectLater(
  ...
  test(
    'manual deload from empty state immediately replaces stale intensity',
    () async {
      final controller = AppController(Repository(_MemoryDatabase()));
      ...
      expect(controller.exerciseStates['squat']!.status, ExerciseStatus.deload);
      expect(
  ...
  group('shared second-session REHIT controller integration', () {
    test(
      'qualifying REHIT recognition handles S2 and preserves S7 semantics',
      () {
        final controller = AppController(Repository(_MemoryDatabase()));
        final now = DateTime.now();
    ...
    test(
      'GREEN strength at exactly 50% exposes the shared eligible result',
      () async {
    ...
    test(
      'controller closes optional REHIT at three cached distinct intensity days but leaves two days eligible',
      () async {
        SessionLog qualifyingRehit(String id, DateTime completedAt) =>
            SessionLog(
              id: id,
              templateId: SessionTypeId.s7,
              tier: SessionTier.full,
    ...
    test(
      'an explicit early end blocks later-day REHIT at exactly 50%',
      () async {
        final controller = await controllerAfterStrength(
          completedSets: 2,
          endedEarly: true,
    ...
    test(
      'YELLOW, RED, and illness guard close an otherwise qualifying day',
      () async {
        final controller = await controllerAfterStrength();
        final now = DateTime.now();
        final plan = strengthPlan();
    ...
    test(
      'pain during first session and a below-50% partial both block',
      () async {
    ...
    test(
      'qualifying S2 structured finisher prevents a duplicate REHIT',
      () async {
        final prescription = cardio.prescriptionFor(
          sessionId: SessionTypeId.s7,
          durationMinutes: 10,
        ...
        expect(reasons, contains(RehitClosedReason.rehitAlreadyCompletedToday));
        expect(
          reasons,
          contains(RehitClosedReason.intensityWithinTrailing48Hours),
        );
    ...
    test(
      'partial S2 finisher blocks next day without earning intensity credit',
      () async {
        final prescription = cardio.prescriptionFor(
          sessionId: SessionTypeId.s7,
          durationMinutes: 10,
        ...
        final log = (await controller.repo.loadSessionLogsSince(
          DateTime(2000),
        )).single;
